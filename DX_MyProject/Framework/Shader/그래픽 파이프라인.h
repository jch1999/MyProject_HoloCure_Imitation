...
(다음 단계들에서 자료들이 어떻게 쓰이는지 알아야 그에 맞는 자료구성을 할 수 있을 것이므로)
...
2D 모니터와 3D 공간의 최대 차이점을 꼽으면, 먼저 3D 공간은 연속적인 무언가로 이뤄져있다고 볼 수 있음
...
일부 설정을 바꾸는 일은 있을 것이기 떄문에 그 때 약간 더 다룰 예정
...
이렇게 색을 결정할 떄, 입력받은 데이터를 그대로 사용할 것인지
혹은 픽셀 위치 등에 따라 모자이크나 흑백이나 세피아튼 등으로 색을 바꾸는 것도 가능하기에
...
Visual Studio의 도구 모음에서 확장->확장 관리->HLSL 검색 -> HLSL for Visual Studio 다운로드(=설치)
