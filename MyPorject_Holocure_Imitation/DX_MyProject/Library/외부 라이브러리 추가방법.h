...
* 외부 라이브러리 추가방법
* 
* A) 이 프로젝트에 라이브러리 파일을 직접 추가하는 방식
* 
* -> 추가하고자 하는 라이브러리 혹은 프레임워크에서 설치하는 방식을 설명해주기도 하므로
* 그 절차를 따라가는 것이 이상적
* 
* 문제는 어지가하면 이런 외부 라이브러리는 대부분 국외에서 만들어진 라이브러리이므로
...
* 
* C) 압축을 푼 파일 안에 있는 .sln 솔루션 파일 중 해당 라이브러리를 사용할 컴퓨터와 같은 사양의 솔루션을 실행
...
* 솔루션 플랫폼을 설정한 뒤, 디버그(F5)를 실행
* 
* E) 컴파일이 완료되면 (파일경로)\DirectXTex.lib 파일을 실행할 수 없다는 에러가 발생함
* 그 에러에서 보여주고 있는 파일 경로를 찾아가, DirectXTex.lib 파일을 복사 후 
* 솔루션 파일이 있는 폴더(DirectTex-Main) 안에 있는 DirexctTex 폴더에 이 파일을 복사
* 
* F) 같은 위치에 있는 DirectTex.h/inl/lib 3개의 파일을 복사한 뒤,
* 우리가 만들고 있는 프로젝트의 Library 폴더에 DirectTex 폴더를 만들고 나서 거기에 붙여녛고 프로젝트에 포함
* 우리가 만들고 있는 프로젝트의 Libarary 폴더에 DirectTex 폴더를 만들고 나서 거기에 붙여넣고 프로젝트에 포함
* 
* G) 미리 컴파일된 헤더에 위 라이브러리를 사용하기 위한 구문을 추가
* 추가하는 위치는 DX 관련 라이브러리를 추가했던 바로 밑
* 
* #pragma comment(lib, "dxguib.lib")
* #include "Libaray/DirectXTex/DirectXTex.h">
...
* 그리고 그게 우리가 만들고 있던 프로젝트와 다를 경우, 맞추는 과정에서 어떤 오류가 생길지 모름
* 
* 따라서 위와 같이 파일을 생성해서 여기에 포함시키는 방식보다,
* 아예 솔류션 자체에 해당 라이브러리 파일을 만드는 프로젝트를 "포함"시켜서
...
*	* 혹여락도 D에서 복사한 뒤에 우리가 사용할 환경의 프로젝트 파일이 남아있다면 제거해도 되고 안 해도 됨
* 
...
* 우리가 해당 라이브러리를 사용할 프로젝트를 클릭한 뒤 DirectXTex가 체크되어있는지 확인
* 
