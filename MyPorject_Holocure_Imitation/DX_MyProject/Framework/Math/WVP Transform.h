...
위 단계들 외에도 원근법/물체의 겹침 등등 시야예 따라 생각해야 하는 것이 매우 많기 때문에
...
해당 벡터 = 해당 좌쵸가 전체 좌표계에서 위치하는 지점까지 변경된다는 의미

따라서 3D 공간에 위치한 오브젝트의 좌표를 바꾸고 싶다면
그 오브젝틀을 표시하는 데 사용된 벡터의 값에 적절한 행렬을 곱해서 벡터의 값을 변환시켜야 함
<- 이 때 사용되는 행렬이 변환 행렬
...
이를 강사의 머리를 기준점으로 놓고 리모콘의 좌표를 구하고자 한다면 특정한 점에서 일정 범위를 회전하고 있다는 등
복잡한 식으로 계산해야 하지만

손을 기준으로 놓고 보면 그냥 손에서 어느 정도 떨어진 거리의 원을 그리며 움직이고 있다는 식으로 아주 간단히 표현 가능
이런 식으로, 전체 공간의 원점을 기준으로 할 경우엔 좌표 처리가 복잡해지지만
그 전체공간 중의 특정한 좌쵸를 새로이 원점으로 집어서 계산할 경우엔 매우 간단해지는 일이 매우 많음
